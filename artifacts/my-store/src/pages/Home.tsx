...
import {
  ShieldCheck, Truck, HeadphonesIcon, ArrowLeft,
  Download, Zap, Code2, FileCode, Lock, Infinity,
  Star, Globe
} from 'lucide-react';
  ...
  return (
    <section className="relative overflow-hidden bg-background text-foreground min-h-[92vh] flex items-center">
      ...
      <div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(0,212,212,0.13) 0%, transparent 70%)' }} />
      ...
      <div className="absolute top-16 left-8 hidden xl:block opacity-60 select-none pointer-events-none">
        <div className="border border-cyan-500/20 bg-black/40 backdrop-blur-sm rounded p-3 text-[11px] font-mono text-cyan-400/70 w-52"
          style={{ boxShadow: '0 0 20px rgba(0,212,212,0.08)' }}>
          <span className="text-purple-400/80">const</span> product = {'{'}<br />
          &nbsp;&nbsp;<span className="text-cyan-300/80">type</span>: <span className="text-green-400/80">"digital"</span>,<br />
          &nbsp;&nbsp;<span className="text-cyan-300/80">download</span>: <span className="text-orange-400/80">true</span><br />
          {'}'}
        </div>
      </div>

      <div className="absolute bottom-24 left-12 hidden xl:block opacity-50 select-none pointer-events-none">
        <div className="border border-purple-500/20 bg-black/40 backdrop-blur-sm rounded p-3 text-[11px] font-mono text-purple-400/70 w-48"
          style={{ boxShadow: '0 0 20px rgba(139,92,246,0.08)' }}>
          <span className="text-cyan-400/80">✓</span> licence.key<br />
          <span className="text-cyan-400/80">✓</span> instant_access<br />
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 hidden lg:flex flex-col gap-3 pl-10 opacity-75 pointer-events-none select-none">
        {[
          { icon: <FileCode className="h-5 w-5" />, name: 'قالب UI احترافي', price: '٨٩', color: 'border-cyan-500/30' },
          { icon: <Code2 className="h-5 w-5" />, name: 'حزمة إضافات Figma', price: '١٢٠', color: 'border-purple-500/30' },
          { icon: <Globe className="h-5 w-5" />, name: 'كورس تصميم كامل', price: '٢٥٠', color: 'border-cyan-400/30' },
          { icon: <Lock className="h-5 w-5" />, name: 'مفتاح ترخيص', price: '٤٥', color: 'border-green-500/20' },
        ].map((item, i) => (
          <div
            key={i}
          ...
          >
            ...
            <span className="mr-auto text-primary font-bold text-xs">{item.price} ر.س</span>
      ...
      <div className="container relative z-10 mx-auto px-4 py-24 flex flex-col lg:flex-row items-center gap-16">
        <div className="w-full lg:w-3/5 space-y-8">
          ...
          {/* Badge */}
          <div className="inline-flex items-center gap-2 rounded border px-3 py-1.5 text-xs font-mono font-semibold"
            style={{
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.15]">
            ...
            <br />
            <span style={{
              background: 'linear-gradient(90deg, hsl(186,100%,52%), hsl(262,68%,72%))',
          ...
          <p className="text-lg text-muted-foreground max-w-xl leading-relaxed">
            قوالب، أدوات، دورات، وملفات رقمية احترافية.
            اشترِ الآن وحمّل مباشرة — بدون انتظار، بدون شحن.
          </p>
          ...
          <div className="flex flex-wrap gap-8 pt-4 border-t"
            ...
            {[
              ...
              { value: '٤.٩', label: 'تقييم المتجر', icon: <Star className="h-3 w-3 fill-current" /> },
              { value: 'فوري', label: 'التسليم' },
            ].map((s, i) => (
              <div key={i} className="flex flex-col">
                <span className="text-2xl font-black flex items-center gap-1"
                  style={{ color: 'hsl(186,100%,52%)', textShadow: '0 0 16px rgba(0,212,212,0.4)' }}>
                  {s.icon}{s.value}
                </span>
        ...
        <div className="hidden lg:block w-2/5 pointer-events-none select-none">
          <div className="border rounded-lg overflow-hidden"
            style={{
              ...
              boxShadow: '0 0 60px rgba(0,212,212,0.1), 0 24px 64px rgba(0,0,0,0.6)',
            }}>
            {/* Terminal header */}
            <div className="flex items-center gap-2 px-4 py-3 border-b"
              style={{ borderColor: 'rgba(0,212,212,0.12)', background: 'rgba(0,0,0,0.3)' }}>
              <div className="flex gap-1.5">
                <div className="w-3 h-3 rounded-full bg-red-500/70" />
                <div className="w-3 h-3 rounded-full bg-yellow-500/70" />
                <div className="w-3 h-3 rounded-full bg-green-500/70" />
              </div>
              <span className="text-xs font-mono text-muted-foreground mr-2">purchase.sh</span>
            </div>
            {/* Terminal body */}
            <div className="p-5 font-mono text-sm space-y-2">
              <div><span className="text-green-400/80">$</span> <span className="text-cyan-300/80">buy</span> <span className="text-foreground/70">--product "UI Kit Pro"</span></div>
              <div className="text-muted-foreground/60 text-xs">جارٍ معالجة الدفع...</div>
              <div className="text-green-400/80">✓ تم الدفع بنجاح</div>
              <div className="text-green-400/80">✓ جارٍ تجهيز الملف</div>
              <div className="text-cyan-400/80">↓ بدء التحميل <span className="animate-pulse">█</span></div>
              <div className="mt-3 border rounded px-3 py-2 text-xs"
                style={{ borderColor: 'rgba(0,212,212,0.2)', background: 'rgba(0,212,212,0.05)' }}>
                <div className="text-muted-foreground/60 mb-1">ui-kit-pro-v2.zip</div>
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.07)' }}>
                  <div className="h-full rounded-full w-3/4"
                    style={{ background: 'linear-gradient(90deg, hsl(186,100%,42%), hsl(262,68%,66%))' }} />
                </div>
                <div className="text-right text-xs mt-1" style={{ color: 'hsl(186,100%,52%)' }}>75%</div>
              </div>
              <div className="text-xs text-muted-foreground/50 mt-2">الوصول مدى الحياة · تحديثات مجانية</div>
            </div>
          </div>
        </div>
      </div>
...
function TrustFeatures({ isDigital }: { isDigital: boolean }) {
  const features = isDigital
    ? [
        { icon: <Download className="h-8 w-8" />, title: 'تحميل فوري', desc: 'استلم ملفاتك فور إتمام الدفع مباشرة' },
        { icon: <Infinity className="h-8 w-8" />, title: 'وصول مدى الحياة', desc: 'تحديثات مجانية وتحميل غير محدود' },
        { icon: <Lock className="h-8 w-8" />, title: 'دفع آمن 100%', desc: 'بياناتك محمية بأحدث معايير التشفير' },
      ]
    : [
        { icon: <ShieldCheck className="h-8 w-8" />, title: 'جودة مضمونة', desc: 'منتجات أصلية 100% ومختارة بعناية' },
        { icon: <Truck className="h-8 w-8" />, title: 'شحن سريع وآمن', desc: 'توصيل لجميع مناطق المملكة' },
        { icon: <HeadphonesIcon className="h-8 w-8" />, title: 'دعم فني متميز', desc: 'متواجدون لخدمتك على مدار الساعة' },
      ];
                ...
                <Link key={category.id} href={`/category/${category.slug}`}
                  ...
                  <div className="relative z-10 transition-transform duration-300 group-hover:-translate-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-white/80 text-sm font-medium">{category.productCount} منتج</span>
